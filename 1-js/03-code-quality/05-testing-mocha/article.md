...
If something is wrong -- then we fix the code, run again, check the result -- and so on until it works for all the cases we verify.

But such manual "re-runs" are imperfect.

**When testing code with manual re-runs, it's easy to miss something.**

For instance, we're creating a function `f`. Wrote some code, testing: `f(1)` works, but `f(2)` doesn't work. We fix the code and now `f(2)` works. Looks complete? But we forgot to re-test `f(1)`. That may lead to an error.

That's very typical. When we develop something, we keep a lot of possible use cases in mind. But it's hard to expect a programmer to check all of them manually after every change. So it becomes easy to fix one case and break another.
...
Let's use a technique named [Behavior Driven Development](http://en.wikipedia.org/wiki/Behavior-driven_development) or, in short, BDD. That approach is used for many projects. BDD is not just about testing. That's more.
